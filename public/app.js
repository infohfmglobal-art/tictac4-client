...
function updateBoard() {
  ...
  for (let i=0;i<cells.length;i++){
    ...
    if (val === "X"){
      cell.textContent = game.skin==="Fruit" ? "🍎" : (game.skin==="Classic"?"X":"🐉");
      cell.classList.add("xMark");
    }
    if (val === "O"){
      cell.textContent = game.skin==="Fruit" ? "🍊" : (game.skin==="Classic"?"O":"🕊️");
  ...
  if (game.winner){
    ...
    confettiBurst();
    screenShake();
    if (game.winner!=="Draw") winSfx.play().catch(()=>{});
  } else {
...
function showHome(show){
  homeScreen.style.display = show?"block":"none";
  ...
  if (show) msgEl.classList.remove("show-winner");
}
showHome(true);
